<#	
	...
	===========================================================================
	 ...
	 Created on:   	11/08/2019 10:21 AM
	 Created by:   	Justin Holmes
	 Organization: 	CANA
	 Filename:     	Energy-AwardProposalProject.ps1
	===========================================================================
	.DESCRIPTION
...
function Start-MainScript
{
	...
	Write-Verbose "Making the new Project Folder"
	."\\canagroup.cana-group\SYSVOL\canagroup.cana-group\scripts\Powershell\Energy\Energy-Wrapper-MakeProjectFolder.ps1"
	
	Write-Verbose "Clearing the screen"
	clear
	Write-Host "-------------------"
	Write-Host "|  Award Project  |"
	Write-Host "-------------------"
	Write-Host ""
	
	
	Write-Verbose "Pausing the screen to the first script to run"
	Write-Host "Please finish with the pop-up window to make the new project folder"
	Read-Host -Prompt "then press any key to continue"
	
	Write-Verbose "Clearing the screen"
	clear
	
	Write-Host "Please select the client folder to work with"
	$ClientName = @(Get-ChildItem \\canagroup.cana-group\business\Energy\Clients | Out-GridView -Title 'Choose a file' -PassThru)
	
	
	Write-Host "Please choose the Proposls Project"
	$ProposlProject = @(Get-ChildItem \\canagroup.cana-group\business\Energy\Clients\$ClientName\Proposals\Active | Out-GridView -Title 'Choose a file' -PassThru)
	
	Write-Host "Please choose the awarded project name that was just created"
	$AwardedProject = @(Get-ChildItem \\canagroup.cana-group\business\Energy\Clients\$ClientName\"Active Projects" | Out-GridView -Title 'Choose a file' -PassThru)
	$SourcePath = "\\canagroup.cana-group\business\Energy\Clients\$ClientName\Proposals\Active\$ProposlProject"
	$DestinationPath = "\\canagroup.cana-group\business\Energy\Clients\$ClientName\Active Projects\$AwardedProject\Project Development\Final Bid Document\$ProposlProject"
	
	Set-Location -Path "\\canagroup.cana-group\business\Energy\Clients\$ClientName\Active Projects\"
	Write-Host "About to copy files of" -NoNewline
	Write-Host " $SourcePath" -NoNewline -ForegroundColor Green
	Write-Host " to" -NoNewline
	Write-Host " $DestinationPath." -ForegroundColor Cyan
	Write-Verbose "About to run robocopy of $SourcePath to $DestinationPath."
	
	$ROBOCOPYOPTIONS = @("/e", "/move", "/r:1", "/w:1", "/v") #| out-null ## Robocopy is dumb and will only accept verables for everything, or nothing
	$ROBOCOPYLOG = "/log:\\canagroup.cana-group\business\Energy\Clients\$ClientName\Active Projects\$AwardedProject\Project Development\Final Bid Document\_copylogs.txt" #| out-null ## Setting the varables for the switches and logfile location
	
	robocopy.exe $SourcePath $DestinationPath $ROBOCOPYOPTIONS $ROBOCOPYLOG
	
	Write-Host " ------------------------- "
	Write-Host " "
	Write-Host "Files have finished coping" -BackgroundColor DarkGreen
	Write-Host "Please check the new folder" -BackgroundColor DarkGreen
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABC