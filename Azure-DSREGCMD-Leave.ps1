<#	
	...
	===========================================================================
	 ...
	 Created on:   	05/19/2020 10:54 AM
	 Created by:   	Justin Holmes
	 Organization: 	CANA IT
	 Filename:     	Azure-DSREGCMD-Leave.ps1
	===========================================================================
	.DESCRIPTION
...
Write-Host "Enter your Administrator Credentials"
$credentials = Get-Credential
$computer = Read-Host "Enter the client Computer Name (i.e. IT-12345)"
$reboot = Read-Host "Do you want to force reboot the client? ($true/$false)"

try
{
	Write-Debug "Pinging $computer for connectivity"
	Test-Connection -ComputerName $computer -ErrorAction Continue
}
catch [Microsoft.PowerShell.Commands.TestConnectionCommand]
{
	Write-Debug "Failed to ping $computer"
	Write-Debug "Continuing anyways"
	Write-Warning "Could not ping $computer"
	Write-Warning "Continuing anyways"
}


try
{
	Write-Debug "Testing WinRM connection"
	Connect-WSMan -ComputerName $computer -ErrorAction Stop
}
catch [System.Management.Automation.Remoting.PSRemotingTransportException],[Microsoft.WSMan.Management.ConnectWSManCommand]
{
	Write-Debug "Failed to connect to the client $computer"
	Write-Warning "Could not connect to $computer!"
	Write-Warning "This could be for a number of reason."
	Write-Warning "1) Computer is offline, or not on the CANA network"
	Write-Warning "2) The DNS entry $computer does not have the current IP address of the client"
	Write-Warning "3) WinRM is not running, or the firewall is blocking the request (TCP:5985)"
}


if ($reboot -eq $true)
{
	Write-Debug "True response for reboot"
	Write-Debug "Going to run the DSREGCMD on $computer"
	Invoke-Command -computername $computer -Credential $credentials -ScriptBlock { & dsregcmd /debug /leave }
	Write-Debug "Going to run the shutdown -r -f -t 30 on $computer"
	Invoke-Command -computername $computer -Credential $credentials -ScriptBlock { & shutdown -r -f -t 30 -c "Rebooting system to finish fix in 30 seconds" }
	Write-Debug "All Done, client should be rebooting"
	Write-Host "All Done, client should be rebooting"
	exit
}
else
{
	Write-Debug "False response for reboot"
	Write-Debug "Going to run the DSREGCMD on $computer"
	Invoke-Command -computername $computer -Credential $credentials -ScriptBlock { & dsregcmd /debug /leave }
	Write-Debug "All Done, client will need to reboot on there own"
	Write-Host "All Done, client will need to reboot on there own"
	exit
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCD6UnrH8Bwu3NR
# DQtP8TK/9wee6PFF+SdeUl+gyTS/UKCCF2owggOwMIICmKADAgECAhBz0HdolQdJ
...
# CSqGSIb3DQEJBDEiBCBotAIqKhZW4VBzDblEMjhz21gpKBwDSvRxieFifp3hDTAN
# BgkqhkiG9w0BAQEFAASCAQB5Fu6176UG1J4FudQy3suHoOWz8A12kL4wBSL7HcXO
# mdZx8TSl9Gf2tMexwLm127ahP+RuwhlhM4rT/Nl3ZpK/I2BPYPlRoAlaAvPIQk4K
# /nJe0C1pvL/AxJKaj98XsF/ykY1dzLyTOgMS0rdYgGiz/BPnwB6MuMkBRwF52jMW
# smO9ycKpThZFswNFcUOGWJyOtCXPLxwnVyck9Z9kDzemTFtphAWP2jSRe6DtOEsR
# drNh4Eyica4RwtJsksZx/8Vmg7jeYP/tWE4D5W1dkWs/UdnMy8cJP7nzHjK4iVX8
# QXoM6fC7AxSMjVq9m4wdgXSc3fvK/P/1T9BSECvZuCIdoYICuTCCArUGCSqGSIb3
...
# MjAxNTU0WjAvBgkqhkiG9w0BCQQxIgQgK3sIjKXpLfvQDR0CY0aIjGtRLYuVniM7
# UIFl9hPweJAwgaAGCyqGSIb3DQEJEAIMMYGQMIGNMIGKMIGHBBQ+x2bV1NRy4hsf
...
# LSBTSEEyNTYgLSBHMgIMJFS4fx4UU603+qF4MA0GCSqGSIb3DQEBAQUABIIBAMvv
# KqilKTPKAmUm/FgrlCI6qrvK0iBZr+8S42uvHkD0yO2LxXZKOBiXuv1Mc8AEp62F
# c6jMBZ0ZD3LY41afEkpmRawZvaAgkIAn8ABqOn5tJHhtjJieYx6ky/j23hyCaUBS
# eWqghpGi7T48ki4mebFQTstNnYv6rSgsAXHv2mOxFTFMsBOWniEWiQY9PdusmCfs
# EN5KH6i6rYqKcyVK4itLq9d3UMYG6PuN3nsEFJAMJYxpYW8mSaWkt4Gvs6gc7iRY
# vlyujTB3h8Ik7N1gjQePu87RKrcOOV2v2HwB2EHvhgBCNtDn88ZwnZjcjd02EjYt
# wit1ZXebAuH/atAxMC0=
