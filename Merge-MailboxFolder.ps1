...
function GetOAuthCredentials
{
    ...
    $platformParameters = New-Object Microsoft.IdentityModel.Clients.ActiveDirectory.PlatformParameters([Microsoft.IdentityModel.Clients.ActiveDirectory.PromptBehavior]::Auto) # Only prompt the user if we need to
...
function ApplyEWSOAuthCredentials
{
    ...
    $exchangeCredentials = GetOAuthCredentials
    ...
    if ($script:sourceService -ne $null)
    {
        $script:sourceService.Credentials = New-Object Microsoft.Exchange.WebServices.Data.OAuthCredentials($script:authenticationResult.Result.AccessToken)
        LogVerbose "Updated OAuth token for source mailbox"
    }

    if ($script:targetService -ne $null)
    {
        $script:sourceService.Credentials = New-Object Microsoft.Exchange.WebServices.Data.OAuthCredentials($script:authenticationResult.Result.AccessToken)
        LogVerbose "Updated OAuth token for target mailbox"
    }
}
