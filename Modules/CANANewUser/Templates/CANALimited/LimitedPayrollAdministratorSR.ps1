<#	
	...
	===========================================================================
	 ...
	 Filename:     	LimitedPayrollAdministratorSR.ps1
	===========================================================================
	.SYNOPSIS
		Adds the user to the groups defined in this function Add-LimitedPayrollAdministratorSR
...
function Add-LimitedPayrollAdministratorSR
{
<#
	.SYNOPSIS
	...
	.EXAMPLE
				PS C:\> Add-LimitedPayrollAdministratorSR $Username
...
#>
	...
	$ADGroups.add('271PayrollAdministrator') | Out-Null
	...
	$ADGroups.add('Acct.CANA.PR') | Out-Null
	...
	$ADGroups.add('Acct.InterCo.PR') | Out-Null
	...
	$ADGroups.add('Acct_PR') | Out-Null
	$ADGroups.add('Acct_PR_Mgr') | Out-Null
	...
	$ADGroups.add('Group_c4d31ab3-1d58-4947-bb24-33605824fa7a') | Out-Null
	...
	$ADGroups.add('Limited_PR_epost_Main') | Out-Null
	$ADGroups.add('MFM Unapproved Time Report') | Out-Null
	$ADGroups.add('Password.Policy.NonExpire') | Out-Null
	$ADGroups.add('Payroll.Contacts.PF') | Out-Null
	$ADGroups.add('PBI_HR') | Out-Null
	$ADGroups.add('portal.FaxPlus') | Out-Null
	$ADGroups.add('Print_HR_Sec') | Out-Null
	$ADGroups.add('SP.All.CHV.R') | Out-Null
	$ADGroups.add('SP.All.Construction.R') | Out-Null
	$ADGroups.add('SP.HRRecords.Managers') | Out-Null
	$ADGroups.add('SP.Payroll.Contribute') | Out-Null
	$ADGroups.add('SP.Payroll.Staff') | Out-Null
	$ADGroups.add('SP_Construction_CRM_Read') | Out-Null
	$ADGroups.add('SP_Payroll_Staff') | Out-Null
	$ADGroups.add('SP_Payroll_Staff') | Out-Null
	$ADGroups.add('stfa.Energy.Regulatory.RWED') | Out-Null
	$ADGroups.add('SW_HFF_TEST') | Out-Null
	$ADGroups.add('SW_Snagit_Users') | Out-Null
	...
	$ADGroups.add('tfa.Accounting.InterCo.PR.RWED') | Out-Null
	...
	$ADGroups.add('Xenapp_Desktop') | Out-Null
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBPrLBKLqj8C+DP
# PajBSYHYCLREKRmomBX7clkeqlgJX6CCJAYwggNfMIICR6ADAgECAgsEAAAAAAEh
...
# BgEEAYI3AgEEMC8GCSqGSIb3DQEJBDEiBCBW9eo+kT62Aec+ZuqydVX3wK93HVGZ
# xukrZzWcXDYMUDANBgkqhkiG9w0BAQEFAASCAQA9Ev+1zCUFW5d0Igz1DweBxbx4
# d8326D6u2U8dvkcG5I3o3p17ihAWnxPXk14OQF6Dk90/QnQWjuRPQ9xxYzDeqrzr
# jMnsjtTNTpeUBv/TtZ1ZFCNyvf2p6cLfXimsMFEed6uKuQxkRuuHnxVR0F640TIB
# OJpKmBVkLMbKFZri/USlJX671cU/16yPVEK+3AKhfkDigMZF51JWe9lBrXkb+Zzk
# lkjlicfxjMWQYHZ/oHTXJwQHzDYZsSc6NC07M2WmN5b97oLtObYRAOIVZ9HlmSpR
# PkxSSNjCGpGqmJPuEzYK+o5/Vz4nl1t8Usho+kva9bILpMfBay7+B+4KMCqBoYID
...
# hvcNAQkFMQ8XDTIxMTAyODE0NDQxOFowLQYJKoZIhvcNAQk0MSAwHjANBglghkgB
# ZQMEAgEFAKENBgkqhkiG9w0BAQsFADAvBgkqhkiG9w0BCQQxIgQgNS6woezY5Wkc
# juwaQ12qJgLEQTDCb7Ev/Wj4BxodHLEwgaQGCyqGSIb3DQEJEAIMMYGUMIGRMIGO
...
# BgkqhkiG9w0BAQsFAASCAYCsAeqKAVR0FrcSjA54cbPj1nO1TGfYSEVgf70HoxOO
# BGgNqIV+NLzUpVyJYZ1o6qy1tZEEGtN1UANe+Uxeq6FeTFfN7tT1H8QfryxwfBrL
# xbv0wNdszVgFPhXFhV5K+5Ch/eamJFZESN7CM2l6cj3i6MRvZAhJBwNXKCU0jnAz
# jYTwLBvjj/uWL3szeh6cirYOu3oHTw19+VuJnVR1KguC9woMAXOZ9gH3QFkt2gH2
# k3xBFNg2eqwbo8Fydk6rSl6DcVPIPC7apaNX9enY7gps6c21cXwuvY0eTwnniSyO
# r32FuWB0wRNv1rXuQa2ZEIOFhk3GVuvDzsIwHzmr9f86cjQX3SOsX/IxiVa5Au5t
# TiIHt1xQ+SNNsoX+S7t6Ao/rpzPcanGJ2YS0P/MRBRP0pdCBbs+YiyCpfkGJKZ/5
# T/7MXRdhBKBJ5d3t9htYCtTBKo+RvJ2CzBgGCjUuJSz9LPkb7h1E3x0e9lFmt3Uq
# +8aHiebQcAOGR68XJf9TmG4=
